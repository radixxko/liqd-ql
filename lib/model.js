...
const getOffset = ( order, row, direction, table ) =>
{
    ...
    for( let k = 0; k < order.length; k++ )
    {
        for( let i = 0; i < order.length - k; i++ )
        {
            ...
            condition += ( !i ? ' ( ' : ' AND ' ) + table + '.' + order_column + ' ' + operator + ' :'+order_column;
        }
...
module.exports = class QLModel
{
    ...
    async get( query, parent )
    {
        ...
        if( this.#scheme[ query.table ])
        {
            ...
            {
                let offset = getOffset( query.orderBy, query.after || query.before, ( query.after ? 'after' : 'before' ), query.table );
