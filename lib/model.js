        ...
        if( this.#tables.has( query.table ))
        {
            ...
            if( parent )
            {
                if( path = this.#relations.path( parent.table, query.table ))
                {
                    ...
                    {
                        let uniqs = parent.rows.map( r => path[0].columns[parent.table].names.reduce(( l, c ) => ( l[c] = r[c], l ), {}));
                        let sql_array = [];

                        for( let i = 0; i < uniqs.length; i++ )
                        {
                            let sub_q = this.#DB.query( [ uniqs[i] ], parent.table )
                                .limit( query.limit ? query.limit : null );

                            path.forEach( node => sub_q.inner_join( node.table, node.condition ));
                            query.condition && query.condition.forEach( c => sub_q.where( query.table + '.' + c[0] + ' ' + c[1] + ' :?', c[2] ));

                            sql_array.push( '( ' + await sub_q.get_all_query( ( parent ? path[0].columns[parent.table].aliases + ', ' : '' ) + columns.map( c => query.table + '.' + c ).join(', ') ) + ' )' );
                        }

                        sql = this.#DB.query( await this.#DB.query().union( sql_array ).get_union( 'a' ) );
                    }
                    else
                    {
            ...
            let select;
            if( parent && query.limit && query.limit > 1 )
            {
                select = await sql.select();
            }
            else
            {
                select = await sql.select(( parent ? path[0].columns[parent.table].aliases + ', ' : '' ) + columns.map( c => query.table + '.' + c ).join(', '));
            }
