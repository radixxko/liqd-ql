...
const OK = async( query ) =>
{
	...
	//LOG.warning( { service: 'test' }, 'QUERY', { data }   );
...
module.exports = class QLModel
{
    ...
    async query( data, id )
	{
		//LOG.warning( { service: 'test' }, 'entry', { data }   );
		...
		if( primary_columns.length === 1 )
		{
			...
			if( id && uniq_ids )
			{
				uniq_ids.forEach( cid => { if( typeof cid === 'object' && cid.hasOwnProperty( primary_columns[0] ) ){ data_uniq_ids.push( cid[ primary_columns[0] ] ); } else if( cid ){ data_uniq_ids.push( cid ); } });

			}

			uniq_column.push( primary_columns[0] );
			columns[ primary_columns[0] ] = primary_columns[0];
			query.where( ( id ? primary_columns[0] + ' IN ( :data_uniq_ids )' : null ), { data_uniq_ids } );
					...
					for( let i = 0; i < path.length; i++ )
                    {
						...
						if( typeof path[i] === 'object' )
                        {
                        	if( Array.isArray( condition.column ) )
							{
								...
								{
									LOG.error( { service: 'test' }, 'START C'   );
                        	...
                        	else
							{
								if( main_table === path[i].table )
								{
									LOG.error( { service: 'test' }, 'START H' , { table, path: path[i] }  );
									models_column[ models[ model ].alias ] = { table: condition.value.table, column: condition.value.column };
				...
				for( let model_name in models_column )
				{
					if( models_column.hasOwnProperty( model_name ) )
					{
						...
						if( Array.isArray( models_column[ model_name ].column ) )
						{
							...
							models_column[ model_name ].column[ model_name ].forEach( row => { multiple_columns[ row ] = getRows.rows[i][ models_column[ model_name ].table + '_'  + row ]; });
				...
				if( models )
				{
					...
					for( let model in models )
					{
						...
						if( sub_models.hasOwnProperty( models[ model ].alias ) )
						{
							...
							promise_all_models.push( ( Service.get( this.#models[ model ] )).query( new_data, model_ids[ models[model].alias ] ) );
						}
						else
						{
		...
		LOG.critical( { service: 'test' }, 'response', { response } );
    ...
    async findPath( a, z )
	{
		LOG.warning( { service: 'test' }, 'path', { a, z }   );
		...
		LOG.warning( { service: 'test' }, 'path', { path }   );
